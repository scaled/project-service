...
object Project {
  ...
  /** Flattens a `Root` to a string. Used to record project metadata. */
  def rootToString (root :Root) = s"${root.path}$Sep${root.testMode}"
  /** Unflattes a `Root` from a string generated by [[rootToString]]. */
  def rootFromString (str :String) = {
    val parts = str.split(Sep)
    Root(Paths.get(parts(0)), if (parts.length > 1) parts(1) == "true" else false)
  }
