...
class Codec (private val idSep :String, private val rootSep :String) {
  import Project._

  // separate Id components by ':' with extra care to avoid ambiguity
  def this () = this(":", "#")
  ...
  /** Converts a [[Root]] to a string. Used to record project metadata. */
  def showRoot (root :Root) =
    root.path + (if (root.testMode) rootSep+"test" else "")
